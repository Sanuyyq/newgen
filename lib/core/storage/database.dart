...
class NegernDb {
  ...
  static Future<void> _onUpgrade(Database db, int oldV, int newV) async {
    ...
    if (oldV < 3) {
      // Подсеиваем дефолтные URL-подписки, если их ещё нет.
      for (final d in kDefaultSubscriptions) {
        final exists = Sqflite.firstIntValue(await db.rawQuery(
              'SELECT COUNT(*) FROM subscriptions WHERE url = ?',
              [d.url],
            )) ??
            0;
        if (exists > 0) continue;
        await db.insert('subscriptions', {
          'engine': d.engine.code,
          'name': d.name,
          'source': SubscriptionSource.url.name,
          'url': d.url,
          'raw_payload': null,
          'last_updated': null,
          'auto_update': 1,
  ...
  Future<void> _seedIfEmpty() async {
    ...
    await _db.transaction((txn) async {
      ...
      // Дефолтные URL-подписки (whitelist). Контент подтянется в фоне.
      for (final d in kDefaultSubscriptions) {
  ...
  Future<List<Subscription>> listAllSubscriptions() async {
    final rows = await _db.query('subscriptions', orderBy: 'id ASC');
