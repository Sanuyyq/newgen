...
class SubscriptionService {
  ...
  /// Меняем подмену клиента «на лету» — будет применено к следующим _fetch.
  void updatePreset(ClientPreset preset) {
    _preset = preset;
  }
  ...
  /// Фоновая синхронизация всех URL-подписок без закешированного содержимого
  /// (например, дефолтных, добавленных при первом запуске).
  /// Ошибки молча игнорируются — это best-effort.
  Future<void> syncPendingUrlSubscriptions() async {
    final all = await _db.listAllSubscriptions();
    for (final sub in all) {
      if (sub.source != SubscriptionSource.url) continue;
      if (sub.url == null || sub.url!.isEmpty) continue;
      if (sub.rawPayload != null && sub.rawPayload!.isNotEmpty) continue;
      try {
        await refresh(sub);
      } catch (_) {
        // best-effort: оставляем подписку «пустой» до следующей попытки.
      }
    }
  }
